...
function tarteaucitron_user_css_js() {
	...
	$allowed 	= array('cs', 'de', 'en', 'es', 'fr', 'it', 'nl', 'pl', 'pt', 'ru', 'el');
    ...
    wp_enqueue_script('tarteaucitron_pro', 'https://opt-out.ferank.eu/tarteaucitron.pro.js?'.$loc.'iswordpress=true&domain='.$domain.'&uuid='.get_option('tarteaucitronUUID'), array(), false, true);
}

add_action('wp_head', 'tarteaucitronForceLocale', 1);
function tarteaucitronForceLocale() {

	$allowed 	= array('cs','de','en','es','fr','it','nl','pl','pt','ru','el','ro','bg','ja','cn');
	$locale 	= substr(get_locale(), 0, 2);
